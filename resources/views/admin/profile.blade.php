...
<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div>
        <h2 class="text-xl font-bold text-gray-800 dark:text-white tracking-tight leading-tight">
            {{ $user->role === 'admin' ? 'Profil Administrator' : 'Pengaturan Mitra' }}
        </h2>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-6 text-center relative overflow-hidden group">
                ...
                <div class="relative z-10">
                    ...
                    <div class="mt-2 inline-flex items-center gap-2 px-3 py-1 rounded-full {{ $user->role === 'admin' ? 'bg-error-50 text-error-600 dark:bg-error-500/10 dark:text-error-400' : 'bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' }}">
                        <div class="w-1.5 h-1.5 rounded-full {{ $user->role === 'admin' ? 'bg-error-500' : 'bg-brand-500' }}"></div>
                        <span class="text-[9px] font-black uppercase tracking-widest">{{ $user->role === 'admin' ? 'Admin' : 'Mitra' }}</span>
                    </div>
                </div>
    ...
    function initMap() {
        ...
        const brandIcon = L.divIcon({
            ...
            html: `<div class="flex items-center justify-center relative" style="width: 44px; height: 44px;">
                        ...
                        <div class="relative flex items-center justify-center shadow-lg" style="background-color:#465fff; width: 34px; height: 34px; border-radius: 50% 50% 50% 0; transform: rotate(-45deg); border: 3px solid white;">
                            <i class="fas fa-map-pin" style="color: white; font-size: 11px; transform: rotate(45deg);"></i>
                        </div>
                   </div>`,
            iconSize: [44, 44],
            iconAnchor: [22, 40]
