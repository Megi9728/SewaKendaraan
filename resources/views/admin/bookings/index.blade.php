...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
    @php
        $stats = [
            ...
            [
                'label' => 'Disewa',
                'value' => $bookings->whereIn('status', ['Active', 'Picked_Up', 'On_the_Way'])->count(),
                'icon' => 'fas fa-car',
                'color' => 'success',
            ],
            [
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800" id="booking-table-body">
                @forelse($bookings as $booking)
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors booking-row" 
                        data-search="{{ strtolower('#TRX-' . $booking->id . ' ' . ($booking->customer->name ?? '')) }}" 
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="text-xs font-bold text-gray-800 dark:text-white truncate tracking-tight mb-0.5">{{ $booking->customer->name ?? 'User Deleted' }}</p>
                                    <p class="text-[9px] font-medium text-gray-400 dark:text-gray-500 truncate leading-none">{{ $booking->customer->email ?? '-' }}</p>
                        ...
                        <td class="py-4 px-6">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <p class="text-[9px] text-blue-600 font-bold uppercase tracking-widest leading-none"><i class="fas fa-handshake mr-1 text-[8px]"></i> {{ $booking->vehicle->mitra->name ?? 'Internal Jatara' }}</p>
                        ...
                        <td class="py-4 px-6">
                            <div class="flex flex-col gap-0.5">
                                ...
                                <div class="text-xs font-bold text-gray-800 dark:text-white mt-1">
                        ...
                        <td class="py-4 px-6 text-center">
                            @php
                                ...
                                $payLabels = ['unpaid' => 'Belum Bayar', 'dp_paid' => 'DP Terbayar', 'fully_paid' => 'Lunas'];
                            @endphp
                        ...
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-1.5">
                                ...
                                @if (in_array($booking->status, ['Active', 'Picked_Up', 'On_the_Way']))
                                    <form action="{{ route('admin.pemesanan.update', $booking->id) }}" method="POST">
                                        @csrf @method('PUT')
                                        <input type="hidden" name="status" value="Completed">
                                        <button type="submit" class="bg-success-500 hover:bg-success-600 text-white text-[8px] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-lg shadow-success-500/20 transition-all active:scale-95 uppercase tracking-widest">
