...
{{-- ===== STAT CARDS ===== --}}
<div class="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-5 mb-8">
    <!-- Revenue -->
    ...
    <!-- Total Mitra -->
    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03] hover:shadow-lg transition-all duration-300 group relative overflow-hidden">
        <div class="absolute top-0 right-0 w-20 h-20 bg-teal-500/5 rounded-full -mr-8 -mt-8 group-hover:scale-110 transition-transform duration-500"></div>
        <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-teal-500/10 text-teal-600 dark:text-teal-400 mb-4 group-hover:scale-110 transition-transform">
            <i class="fas fa-handshake text-lg"></i>
        </div>
        <div>
            <span class="text-[9px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Total Mitra</span>
            <h4 class="text-lg font-bold text-gray-800 dark:text-white mt-1">
                {{ $stats['total_mitra'] }} <span class="text-[10px] font-medium opacity-50 ml-1">Mitra</span>
            </h4>
        </div>
        <div class="mt-3 flex items-center gap-2 text-[9px] font-bold text-teal-500 bg-teal-500/10 px-2 py-0.5 rounded-md w-fit">
            TERDAFTAR
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03] hover:shadow-lg transition-all duration-300 group relative overflow-hidden">
        ...
        <div>
            <span class="text-[9px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Disewa</span>
            <h4 class="text-lg font-bold text-gray-800 dark:text-white mt-1">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-white dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden p-8">
            <div class="flex justify-between items-center mb-8">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-sm font-bold text-gray-800 dark:text-white uppercase tracking-wider">
                        Performa Transaksi Mitra
                    </h3>
                </div>
                <a href="{{ route('admin.mitra.index') }}" class="text-[10px] font-bold text-brand-600 dark:text-brand-400 uppercase tracking-widest hover:underline">Kelola Mitra <i class="fas fa-arrow-right ml-1"></i></a>
            </div>
                    ...
                    <table class="w-full text-left whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors">
                                ...
                                <td class="py-3 px-6 text-sm font-medium text-gray-600 dark:text-gray-400 text-center">
    ...
    <div class="lg:col-span-1 space-y-8">
        ...
        {{-- Status Armada --}}
        <div class="bg-white dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-8">
            ...
            <div class="space-y-6">
                @php
                    $totalV = $stats['total_vehicles'] ?? 0;
                    $rentedV = $stats['rented_vehicles'] ?? 0;
                    $availablePct = $totalV > 0 ? round(($totalV - $rentedV) / $totalV * 100) : 0;
                    $rentedPct = $totalV > 0 ? round($rentedV / $totalV * 100) : 0;
                @endphp
                <div>
                    <div class="flex justify-between items-center mb-3">
                        <p class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Tersedia untuk Sewa</p>
                        <p class="font-bold text-gray-800 dark:text-white text-sm">{{ $totalV - $rentedV }} <span class="text-[10px] text-gray-400 opacity-50">Unit</span></p>
                    </div>
                    <div class="h-2 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                        <div class="bg-success-500 h-full rounded-full transition-all duration-1000" style="width: {{ $availablePct }}%"></div>
                    </div>
                </div>

                <div>
                    <div class="flex justify-between items-center mb-3">
                        <p class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Sedang Beroperasi</p>
                        <p class="font-bold text-gray-800 dark:text-white text-sm">{{ $rentedV }} <span class="text-[10px] text-gray-400 opacity-50">Unit</span></p>
                    </div>
                    <div class="h-2 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                        <div class="bg-brand-500 h-full rounded-full transition-all duration-1000" style="width: {{ $rentedPct }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 dark:border-gray-800 flex flex-wrap gap-4">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Ready</span>
                </div>
                <div class="flex items-center gap-2">
                    <div class="w-2.5 h-2.5 rounded-full bg-brand-500"></div>
                    <span class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Disewa</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-white/[0.03] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm p-8">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-gray-800 hover:border-brand-500/30 transition-all group">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-lg bg-teal-500/10 text-teal-600 dark:text-teal-400 flex items-center justify-center font-black uppercase text-xs">
                            {{ substr($mitra->name, 0, 1) }}
                        </div>
                        <div>
                            <p class="text-xs font-bold text-gray-800 dark:text-white">{{ $mitra->name }}</p>
                            <p class="text-[10px] text-gray-400 dark:text-gray-500 font-medium">{{ $mitra->vehicles_count }} Armada</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-sm font-bold text-brand-500">{{ $mitra->bookings_count }}</p>
                        <p class="text-[8px] text-gray-400 uppercase tracking-widest font-bold">Orders</p>
                    </div>
                </div>
                @empty
                <div class="text-center py-6 text-gray-400">
                    <p class="text-xs font-medium">Belum ada mitra terdaftar.</p>
                </div>
                @endforelse
            </div>
            <a href="{{ route('admin.mitra.index') }}" class="mt-8 block w-full py-3 bg-gray-50 dark:bg-white/5 text-center text-[10px] font-bold text-gray-800 dark:text-white rounded-xl hover:bg-brand-500 hover:text-white transition-all uppercase tracking-widest">Kelola Semua Mitra</a>
        </div>
    </div>
