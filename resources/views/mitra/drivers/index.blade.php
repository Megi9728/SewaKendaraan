    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Driver & Email</th>
                    ...
                    <td class="px-6 py-5">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-blue-600 font-bold lowercase tracking-wider">{{ $d->user->email ?? '-' }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-5">
...
<div id="modal-driver" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
    ...
    <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg z-10 overflow-hidden">
        ...
        <form id="driver-form" action="{{ route('mitra.drivers.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-5">
            ...
            <div>
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Email Akun (Untuk Login)</label>
                <input type="email" name="email" id="f-email" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3.5 text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400 transition-all" placeholder="driver@email.com">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Password Akun</label>
                    <input type="password" name="password" id="f-password" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3.5 text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400 transition-all" placeholder="Kosongkan jika tidak ganti">
                </div>
                <div>
...
<script>
    ...
    function openEditModal(driver) {
        ...
        document.getElementById('f-email').value = driver.user ? driver.user.email : '';
